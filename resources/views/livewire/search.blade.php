<div class="relative border border-white bg-black p-1 text-white">
  ...
  <input wire:model.live.debounce.250ms="search" type="text" placeholder="Titles, people, genres" class="ml-8 bg-black text-white placeholder-gray-500 focus:outline-none">
  ...
  <div class="absolute mt-2 -ml-2 w-64 bg-gray-700 p-2 text-sm">
    ...
    <ul>
      ...
      @if ($initialRouteName === 'stream.index')
      <a href="{{ route('movies.show', $result['id']) }}" class="hover:bg-gray-400">
        <li class="border-b border-gray-500 p-1">
          {{ $result['title'] }}
        </li>
      </a>
      @elseif ($initialRouteName === 'shows.index')
      <a href="{{ route('shows.show', $result['id']) }}" class="hover:bg-gray-400">
        <li class="border-b border-gray-500 p-1">
          {{ $result['name'] }}
        </li>
      </a>
